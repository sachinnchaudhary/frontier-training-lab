...
def gelu_ffn_params(dim: int, hidden_dim: int) -> int:
    return dim * hidden_dim + hidden_dim + hidden_dim * dim + dim


def swiglu_ffn_params(dim: int, hidden_dim: int) -> int:
    return 2 * (dim * hidden_dim + hidden_dim) + hidden_dim * dim + dim


def matched_swiglu_hidden_dim(dim: int, gelu_hidden_dim: int) -> int:
    target = gelu_ffn_params(dim, gelu_hidden_dim)
    best_hidden = 1
    best_error = abs(swiglu_ffn_params(dim, best_hidden) - target)
    for hidden_dim in range(2, gelu_hidden_dim + 1):
        error = abs(swiglu_ffn_params(dim, hidden_dim) - target)
        if error < best_error:
...
GELU_HIDDEN_DIM = 512
SWIGLU_HIDDEN_DIM = matched_swiglu_hidden_dim(dim=512, gelu_hidden_dim=GELU_HIDDEN_DIM)

RUNS = [
    FFNRunConfig(
        run_name="gelu_hidden512",
        ffn_type="gelu",
        ffn_hidden_dim=GELU_HIDDEN_DIM,
    ),
    FFNRunConfig(
        run_name=f"swiglu_hidden{SWIGLU_HIDDEN_DIM}_matched",
        ffn_type="swiglu",
        ffn_hidden_dim=SWIGLU_HIDDEN_DIM,
    ),
]
...
def ffn_params_for(config: FFNRunConfig) -> int:
    if config.ffn_type == "gelu":
        return gelu_ffn_params(config.dim, config.ffn_hidden_dim)
    if config.ffn_type == "swiglu":
        return swiglu_ffn_params(config.dim, config.ffn_hidden_dim)
    raise ValueError(f"unknown ffn_type: {config.ffn_type}")


def metadata(config: FFNRunConfig, param_count: int) -> dict:
    ffn_params = ffn_params_for(config)
    return {
        "ffn_type": config.ffn_type,
        "ffn_hidden_dim": config.ffn_hidden_dim,
        "ffn_parameters_per_layer": ffn_params,
        "ffn_parameters_total": ffn_params * config.num_layers,
...
def run_experiment(config: FFNRunConfig, output_dir: Path) -> dict:
    ...
    for step in range(config.max_steps + 1):
        ...
        if step % config.log_interval == 0:
            ...
            print(
                ...
                f"ffn_hidden={config.ffn_hidden_dim} "
...
def parse_args():
    parser = ArgumentParser(description="Run GELU vs SwiGLU FFN sweep.")
    parser.add_argument(
        "--max-steps",
        type=int,
        default=None,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("experiment/ffn_runs"),
