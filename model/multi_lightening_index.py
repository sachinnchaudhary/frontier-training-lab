...
class LightningSparseConfig:
    ...
    # logsumexp compression sharpness
    lse_alpha: float = 4.0

    # auxiliary indexer learning
    index_aux_weight: float = 0.02
    teacher_temp: float = 1.0
    student_temp: float = 1.0

    # optional differentiable bias into selected attention logits
    # keep 0.0 for clean first experiments
...
def lightning_index_scores(x, params, config):
    """
    Multi-perspective Lightning Indexer.

    Produces one scalar sparse-retrieval score per query/key token pair:
        scores: [B, T, T]

    The compression is:
        score(t, k) = (1 / alpha) logsumexp_i(
            log gate_i(t) + alpha * ReLU(tau_i * <q_i(t), k(k)> / sqrt(index_dim))
        )
    """
    B, T, _ = x.shape
    ...
    # Clamp tau to prevent runaway sharpness.
    log_tau = jnp.clip(params["idx_log_tau"], -2.0, 2.0)
    tau = jnp.exp(log_tau).astype(x.dtype)
...
def indexer_auxiliary_loss(index_score, q, kv_key, config):
    """
    Differentiable teacher loss for the indexer.

    The sparse top-k path itself is hard and non-differentiable.
    This auxiliary loss teaches the indexer to approximate the dense
    attention distribution induced by the current MLA Q/K geometry.
...
def lightning_sparse_attention(x, params, config, return_aux=False):
    """
    Perspective-Preserving Lightning Sparse Attention.

    Main sparse MLA interface stays unchanged:
        indexer -> one top-k token set per query -> normal sparse attention

    Correction versus the earlier version:
        the indexer also receives a differentiable auxiliary loss,
        because hard top-k indices alone do not backprop into idx_q/idx_k/router.
    """
    ...
    if config.index_score_bias_beta != 0.0:
        ...
        selected_index_score = jnp.where(selected_valid, selected_index_score, 0.0)
    ...
    # Keep your simple FFN path. No MoE.
...
if __name__ == "__main__":
    ...
    def tiny_loss(p):
        y, index_aux = lightning_sparse_attention(x, p, config, return_aux=True)
    ...
    for name in ["idx_q", "idx_k", "idx_router", "idx_log_tau"]:
